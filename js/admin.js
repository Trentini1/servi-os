...
window.AdminView = ({ setView, showAlert, showConfirm }) => {
                        ...
                        users.map(u => (
                            <div key={u.id} className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-800/80 transition-colors">
                                <div className="flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-white text-sm shadow-inner
                                ...
                                <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-700 w-full sm:w-auto">
                                    <button 
                                        ...
                                        className={`flex-1 sm:flex-none px-3 py-1.5 text-[10px] font-bold uppercase rounded transition-colors ${u.role === 'client' ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}
                                    >Cliente</button>
                                    <button 
                                        onClick={() => changeRole(u.id, 'tech')}
                                        className={`flex-1 sm:flex-none px-3 py-1.5 text-[10px] font-bold uppercase rounded transition-colors ${u.role === 'tech' ? 'bg-blue-600 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}
                                    >Técnico</button>
                                    <button 
                                        onClick={() => changeRole(u.id, 'master')}
                                        className={`flex-1 sm:flex-none px-3 py-1.5 text-[10px] font-bold uppercase rounded transition-colors ${u.role === 'master' ? 'bg-purple-600 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}
                                    >Master</button>
                                </div>
                            </div>
                        ))
